...
{
    //[AbpMvcAuthorize]
    public class MoviesController : MachineLearningBPControllerBase
    {
        readonly IRepository<NbaGame> _movieRepository;
        readonly IObjectMapper _objectMapper;

        public MoviesController(IRepository<NbaGame> movieRepository, IObjectMapper objectMapper)
        {
        ...
        public ActionResult Today_Read([DataSourceRequest] DataSourceRequest request)
        {
            ...
            result.Data = _objectMapper.Map<List<NbaGameDto>>(_movieRepository.GetAllIncluding(x => x.StatLines.Select(y => y.Participant)).Where(request.Filters).Where(todayFunc).OrderBy(request.Sorts[0]).ToList());
            result.Total = _movieRepository.GetAll().Where(request.Filters).Where(todayFunc).Count();
